...
class Index extends Base
{
    ...
    public function countrygroup($appid = "", $start = "", $end = "")
    {
        ...
        $rois = "";
        $common_roi = "";
		$country = Db::name("adspend_data")->field('country')->where(" app_id={$appid} and spend>10 and date>='{$start}' and date<='{$end}'")->group('country')->select();
...
FROM hellowd_summary_data WHERE date>='{$start}' and date<='{$end}' GROUP BY app_id ORDER BY (revenue*0.2/roi*0.8) desc) AS t;";
	...
	private function get_total_ltv($list)
    {
        ...
        return ["ltv"=>round($ltv,2),"ltv_ad_revenue"=>round($ltv_ad_revenue,2),"ltv_purchase"=>round($ltv_purchase,2)];
    }
    ...
    public function summary_json($start = "", $end = "", $country = "all")
    {
        ...
        $total_data["roi0"] = $total_data["spend"]["spend"]>0?round($ltv_revenue*100/$total_data["spend"]["spend"],2):"0";
    ...
    public function copartner_new_json($start = "", $end = "")
    {
        ...
        foreach ($dates as $k => $v) {
            ...
            $vv = ["date" => $v, "int_show" => $int_show, "rew_show" => $rew_show, "ban_show" => $ban_show, "active_users" => $active_users, "new_users" => $new_users, "reten_1" => round($reten, 2), "spend" => round($spend["spend"], 2), "revenue" => round($revenue['total']['revenue'], 2), "roi" => $roi, "ban_revenue" => $ban_revenue, "int_revenue" => $int_revenue, "rew_revenue" => $rew_revenue];
    ...
    public function gettotaldata($appid = "", $start = "", $end, $country = "", $from = "1")
    {
        ...
        return ["purchase" => $purchase, "roi" => $roi, "reten" => $reten, "new_users" => $new_users, "revenue" => $revenue, "spend" => $spend, "nature_num" => $nature_num, "nature_rat" => $nature_rat,"ltv_ad_revenue"=>$ltv_total_revenue["ad_revenue"],"ltv_purchase"=>$ltv_total_revenue["purchase"],"ltv_ad0"=>$ltv_ad0,"ltv_pr0"=>$ltv_pr0,"ltv0" => $ltv0,"ltv_revenue"=>$ltv_revenue,"roi0" => $roi0];
    }
