...
class Product extends Base
{
	...
	public function index($appid="",$start="",$end="",$groupby="product",$select="all")
    {      	    
	   $t1 = microtime(true);
	  ...
	  if( $groupby=="product" ){
        if( $start=="" || $end=="" )
		{
		  ...
		  $r = Db::name("summary_data")->field('app_id,sum(spend) as spend,sum(revenue) as revenue,sum(new_users) as new_users,sum(nature_num) as nature_num,avg(active_users) as avg_active_users,sum(active_users) as active_users,sum(purchase) as purchase,round(avg(reten),2) as reten')->where(" date>='{$start}' and date<='{$end}' and revenue>0")->group('app_id')->select();
		  if( !empty($r) )
		  {
			  ...
			  foreach( $r as $key=>&$v )
			  {
					...
					if($admin_id==32 || $admin_id==59)
					{
						$type = $admin_id==32?1:2;
						if($rr["app_base_id"])
						{
				   ...
				   $out_data[$key]["icon_url"] = $v["icon_url"];
	  ...
	  $cooperate_revenue = $this->cooperate($start,$end);
	  $rebate_revenue = $this->getRebate($start,$end);
	...
	private function newgetproductday($start,$end){
		...
		$res = Db::name("summary_data")->field("revenue,spend,app_id")->where($where." and ( spend>0 or revenue>0)")->select();
		$total_spend ="0.00";
        $total_revenue ="0.00";
		$roi="0.00";
		if(!empty($res))
		...
		return ["roi"=>$roi,"revenue"=>$total_revenue,"spend"=>$total_spend];
	}
	...
	private function getdaydata($start, $end)
	{
		...
		return ["list"=>$out_data,"total_revenue"=>$total_revenue,"total_spend"=>$total_spend,"total_purchase"=>$total_purchase];
	}
	...
	//返点收益计算
	private function getRebate($start,$end){
...
WHERE sps.platform_type=5 and sps.date>='{$start}' and sps.date<='{$end}' GROUP BY aa.`subject`,sps.country";
		...
		$fb_data = Db::name("adspend_data")->field("sum(spend) as spend")->where("platform_type=6 and date>='{$start}' and date<='{$end}'")->find();
        $revenue+=($fb_data["spend"]>0?round($fb_data["spend"]*0.08,2):"0.00");		
		$toutiao_data = Db::name("adspend_data")->field("sum(spend) as spend")->where("platform_type=32 and date>='{$start}' and date<='{$end}'")->find();
		$tittok = [
...
join hellowd_advertising_account aa ON sps.target_id=aa.advertiser_id 
WHERE sps.platform_type=36 and sps.date>='{$start}' and sps.date<='{$end}' GROUP BY aa.`subject`,sps.country";
	...
	//分成
	private function cooperate($start,$end){
		$res = Db::name("revenue_rate")->field("app_id,revenue_rate")->where( "app_id in(77,143,147) and app_id is not null" )->select();
		$revenue="0.00";
		if(!empty($res))
		{
			foreach($res as &$v)
