...
class Index extends Base
{
    ...
    {      
		...
		{
			return $this->revenue($appid,$start,$end,$platform,$country);exit;
		}
		if( $role=="advertiser" )
		{			
			...
			{
				return $this->spend($appid,$start,$end);exit;
			}
		...
		if( $role=="material" )
		{
			return redirect('/admin_adset/index');exit;
		}
	...
	public function copartner($appid="",$start="",$end="")
	{
		...
		$this->assign("appid",$appid );
		$this->assign("month_revenue",$month_revenue );
	...
	public function countrygroup($appid="",$start="",$end="")
	{
		...
		$rois="";
		$common_roi="";
		foreach($allcountrys as $k=>$v)
		{
			$out_data[$k]["name"] =$v;           			
            $data = $this->getmaintotal($appid,$start,$end,$k);
			if( $k!="all" &&  $data["roi"]<10000)
			{
				$echats_data[$k] =[ "name"=>$v,"roi"=>$data["roi"],"spend"=>$data["spend"]["spend"],"revenue"=>$data["revenue"]["total"]["revenue"] ];
			}
...
FROM hellowd_summary_data WHERE date>='{$start}' and date<='{$end}' GROUP BY app_id ORDER BY (revenue*0.2/roi*0.8) desc) AS t;";
	...
	//对所选日期汇总
	private function getmaintotal($appid,$start,$end,$country)
	{
	...
	public function copartner_json($start="",$end="")
	{
		...
		foreach( $dates as $k=>$v )
		{
			...
			$int_revenue = $impression['int']['revenue'];
			$rew_revenue = $impression['rew']['revenue'];
			$ban_revenue = $impression['ban']['revenue'];
			$reten = $this->getreten($appid,$v,$v,"all");
			if( $appid==112 )
			{				
				if( strtotime("2019-06-18 23:59:59") < strtotime($v) &&  strtotime($start)< strtotime("2019-07-15 23:59:59") )
				{
					$new_users  =ceil( $new_users*0.8);
					$active_users = ceil( $active_users*0.8);
				}				
	...
	public function addMoney($id="",$money="",$desc="")
	{
		if( $id )
		{
			Db::name("copartner_month")->where(["id"=>$id])->update(["money"=>$money,"desc"=>$desc,"status"=>2]);
			exit("ok");
		}
		exit("fail");
	}
	...
	{
		$revenue = $this->getrevenuetotal($appid,$start,$end,"all",$country);
        ...
        if( $appid=="112" && $from=="1" )
		{
			...
			if( $start==$end )
			{
				if( strtotime("2019-06-18 23:59:59") < strtotime($start) &&  strtotime($start)< strtotime("2019-07-15 23:59:59") )
				{
					$new_users  =ceil( $new_users*0.8);
					$revenue["total"]["active_users"] = ceil( $revenue["total"]["active_users"]*0.8);
				}
			}
		    
			$w_data = Db::name("walk_data")->field("sum(revenue) as revenue,sum(spend) as spend")->where( "appid={$appid} and country='{$country}' and date>='{$start}' and date<='{$end}'" )->find();
			if( !empty($w_data) )
			{
				
				if(  $start!=$end )
				{
					if( strtotime("2019-05-28 00:00:00") < strtotime($start) )
					{
						$revenue["total"]["revenue"]=round($w_data["revenue"],2);				
				        $spend["spend"]=round($w_data["spend"],2);
					}
				}else{
					$revenue["total"]["revenue"]=round($w_data["revenue"],2);				
				    $spend["spend"]=round($w_data["spend"],2);
				}						
				$revenue["total"]["dauarpu"] =$revenue["total"]["active_users"]>0?round($revenue["total"]["revenue"]/$revenue["total"]["active_users"],3):0;				
				$spend["cpi"] = $new_users>0?round($spend["spend"]/$new_users,2):0;
			}
		}			
		...
		return [ "purchase"=>$purchase,"roi"=>$roi,"reten"=>$reten,"new_users"=>$new_users,"revenue"=>$revenue,"spend"=>$spend,"nature_num"=>$nature_num,"nature_rat"=>$nature_rat ];
	}
	
	private function getWalkData($appid)
	{
	  $t = Db::name("walk_data")->field('sum(spend) as spend,sum(revenue) as revenue')->where( "appid={$appid}" )->find();
	  return $t;
	}
	...
	public function revenue($appid="",$start="",$end="",$platform="all",$country="all")
	{
		...
		$userinfo = getuserinfo();
		$allow_list = ["producter","super","publisher","advertiser"];
	...
	private function getEcpmArpdau($data)
	{
		...
		if( !empty($data) )
		{
			foreach($data as $key=>$vv)
			{
				$vv["date"] = date("m月d日",strtotime($vv["date"]));
				$date.="'{$vv["date"]}',";
				$ecpm.=$vv["result"]["total"]['ecpm'].",";
				$arpdau.=$vv["result"]["total"]['dauarpu'].",";
				$new_users.=$vv["new_users"].",";
			}
			$ecpm = rtrim($ecpm,",");
			$arpdau = rtrim($arpdau,",");
			$new_users = rtrim($new_users,",");
		}
		return ["date"=>rtrim($date,","),"ecpm"=>$ecpm,"arpdau"=>$arpdau,"new_users"=>$new_users ];
	}
	...
	private function getrangerevenueday($appid,$start="",$end="",$platform="all",$country="all")
	{
		...
		foreach( $dates as $k=>$v )
		{
			...
			$out_data[$k]["new_users"] =$this->getnew_users($appid,$v,$v,$country);
			$out_data[$k]["publisher_showtips"] = isshowtips($appid,$v,"publisher");	
		}
		return $out_data;
	}
	...
	private function get_by_country_spend($appid,$start="",$end="",$country="all",$platform="all")
	{
		...
		if($platform=="all")
		{
			...
			if( !empty($d) )
			 {
				 $d = $d[0];
				 $r["spend"] = $d["spend"]?$d["spend"]:"0.00";
				 $r["installs"] = $d["installs"]?$d["installs"]:0;		
			 }
	...
	{
		$userinfo = getuserinfo();
		$all_where="1=1 and id in(83,81,77,68,66,52,69,70,31)";
		...
		$res = Db::name('app')->where($all_where)->select();
		$this->assign("res",$res);
		return $this->fetch();
	}
