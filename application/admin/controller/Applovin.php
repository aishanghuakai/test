...
class Applovin extends Base
{
    ...
    private function getpurchasev1($app_id = "", $install_date, $event_start_date, $event_end_date, $campaign_id = "",$adset_id="",$country = [], $media_source = [])
    {
        ...
        if (!empty($d)) {
            return $d["revenue"] ? $d["revenue"] * 0.7 : "0.00";
        }
        return "0.00";
    }
    ...
    public function get_v1_ltv_data($app_id = "", $start, $end, $country, $day = "", $day_num = "", $media_source, $is_device = false, $device_category = "")
    {
		...
		if($day==0)
		{
			if(!$this->isd($end,3))
			{
				...
				$out["cus_total_roi1"] = $spend["spend"] > 0 ? round($r3["new_total_revenue"] * 100 / $spend["spend"], 2) : 0;
			}else{
				$out["cus_total_roi1"]="--";
			}
			...
			if(!$this->isd($end,7))
			{
				...
				$out["cus_total_roi2"] = $spend["spend"] > 0 ? round($r7["new_total_revenue"] * 100 / $spend["spend"], 2) : 0;
			}else{
				$out["cus_total_roi2"]="--";
			}			
		}
		if($day==3)
		{
			...
			{
				$r7 = $this->get_v2_one_day_ltv($app_id, $spend, $start, $end,7, $day_num, $country, $media_source, $device_category);
				$out["cus_total_roi1"] = $spend["spend"] > 0 ? round($r7["new_total_revenue"] * 100 / $spend["spend"], 2) : 0;
			}else{
				$out["cus_total_roi1"]="--";
			}			
			if(!$this->isd($end,10))
			{
				...
				$out["cus_total_roi2"] = $spend["spend"] > 0 ? round($r10["new_total_revenue"] * 100 / $spend["spend"], 2) : 0;
			}else{
				$out["cus_total_roi2"]="--";
			}
		}
		if($day==7)
		{
			...
			{
				$r10 = $this->get_v2_one_day_ltv($app_id, $spend, $start, $end,10, $day_num, $country, $media_source, $device_category);
				$out["cus_total_roi1"] = $spend["spend"] > 0 ? round($r10["new_total_revenue"] * 100 / $spend["spend"], 2) : 0;
			}else{
				$out["cus_total_roi1"]="--";
			}
			...
			if(!$this->isd($end,14))
			{
				...
				$out["cus_total_roi2"] = $spend["spend"] > 0 ? round($r14["new_total_revenue"] * 100 / $spend["spend"], 2) : 0;
			}else{
				$out["cus_total_roi2"]="--";
			}
		}
...
from  hellowd_adjust_ltv_users  WHERE {$where} GROUP BY install_date order by install_date desc");
    ...
    private function get_v2_revenue($app_id, $install_date, $event_start_date, $event_end_date, $country, $media_source, $device_category)
    {
		...
		if ($event_start_date<'2020-09-01'){
			...
			$new_row = Db::name("applovin_max")->field('sum(revenue) as revenue,sum(int_show) as int_show,sum(rew_show) as rew_show')->where($where)->find();
			$row = $new_row;
		}	
...
GROUP BY install_date,event_date,ad_id,gb_id,country";
    ...
    private function get_campaign_revenue($app_id, $campaign_id, $install_date, $event_start_date, $event_end_date, $country, $media_source)
    {
        ...
        $new_row = Db::name("campaign_max")->field('sum(revenue) as revenue,sum(int_show) as int_show,sum(rew_show) as rew_show')->where($where)->find();
		$row = $new_row;
    ...
    private function get_ad_revenue($app_id, $ad_id, $install_date, $event_start_date, $event_end_date, $country, $media_source)
    {
        ...
        $row = Db::name("ad_max")->field('sum(revenue) as revenue,sum(int_show) as int_show,sum(rew_show) as rew_show')->where($where)->find();
        if (!empty($row)) {
            $row['ad_revenue'] = $row["revenue"];
            $row['purchase'] = '0.00';
            return $row;
    ...
    public function campaign_ltv_json()
    {
        ...
        $list = $this->get_campaign_list($params['app_id'], $start, $end, $params['media_source']);
        ...
        if (!empty($list)) {
            foreach ($list as $v) {
				...
				$row["hasChildren"] = true;
				$row["id"] = uniqid();
				$cam = $db->table("ads_online_info_campaign")->field('status')->where(["campaign_id"=>$v["campaign_id"]])->find();
				$row["campaign_status"] = $cam["status"];
	...
	public function get_adset_ltv_total_data()
	{
		...
		$spend = $this->get_v1_sepend($params['app_id'],$start,$end,$params['country']=='all'?[]:[$params['country']],$media_channel);
		$row = $this->get_adset_ltv_data($params['app_id'],$spend,"",$start,$end,$params['country'],$params['day'], $params['media_source']);
		echo json_encode($row);
        exit;
	}
    ...
    public function adset_ltv_json()
    {
        ...
        $list = $this->get_adset_list($params['app_id'], $start, $end,$params['country'],$params['media_source']);
        $out = [];
        if (!empty($list)) {
            foreach ($list as $v) {
    ...
    private function get_campaign_list($app_id, $start, $end, $media_source)
    {
        $where = "install_date>='{$start}' and install_date<='{$end}' and gb_id={$app_id} and media_source='{$media_source}'";
        $res = Db::name("campaign_max")->field("campaign_id,campaign_name")->where($where)->group('campaign_id')->select();
        return $res;
    }

    private function get_adset_list($app_id, $start, $end, $country,$media_source)
    {
        $media_channel = $this->getchannel($this->promate_media, $media_source);		
        ...
        $res = Db::name("adspend_data")->field("adset_id,sum(spend) as spend,ceil(sum(installs)) as installs")->where($where)->group('adset_id')->select();
        return $res;
    }
	...
	public function get_ltv_roi($date="",$country="",$appid=""){
		...
		$ad_data = Db::name("applovin_max")->field("SUM(revenue) as revenue")->where($where)->find();
	...
	public function revenue_ltv_json($date="",$group="",$country="")
	{
		...
		switch($group)
		{
			case 'day':
			 ...
			 $sql ="SELECT install_date as name,SUM(revenue) as revenue,round(sum(int_show+rew_show),2) as total_show,round(SUM(int_show),2) as int_show,round(SUM(rew_show),2) as rew_show  from hellowd_applovin_max WHERE {$where} GROUP BY install_date ORDER BY install_date desc";
			break;
			case 'channel':
			 $columns =["渠道","广告收益","收益占比"];
			 $sql ="SELECT media_source as name,SUM(revenue) as revenue  from hellowd_applovin_max WHERE {$where} GROUP BY media_source order by revenue desc";
			break;
			case 'device':
			 $columns =["设备类型","广告收益","收益占比"];
			 $sql =" SELECT device_category as name,SUM(revenue) as revenue  from hellowd_applovin_max WHERE {$where} GROUP BY device_category";
			break;
			case 'roi':
		...
		if(!empty($data))
		{
			$res = Db::name("applovin_max")->field("SUM(revenue) as revenue")->where($where)->find();
