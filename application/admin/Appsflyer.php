...
class Appsflyer extends Base
{
	...
	public function json_data($app_id="",$date="",$country="all",$day=[],$media_source="all",$ad_source="all",$campaign_id="",$device="all")
	{
		//$start = $date;
		if(!$date)
		{
			$date = date("Y-m-d",strtotime("-15 day"));
			$range_date =[$date,$date];
		}
		if( empty($day) )
		{
			$start = $date;
			$end =date("Y-m-d",strtotime($start)+(86400*30));
			//$end = date("Y-m-d",strtotime("-1 day"));
		}else{
			list($start,$end) = $day;
		}
		
		//$end =date("Y-m-d",strtotime($date)+(86400*$day));
		$dates = getDateFromRange($start, $end);
		$media_channel =$this->getchannel($this->promate_media,$media_source);
		
		$spend = $this->getsepend($app_id,$date,$date,$country,$media_channel);
		$cpi = $spend["installs"]>0?round($spend["spend"]/$spend["installs"],2):"0.00";
		$out_data =[];
		$total_revenue ="0.00";
		foreach( $dates as $key=>$v )
		{
			$r= $this->get_one_rows($app_id,$date,$v,$v,$country,$media_source,$ad_source,$campaign_id,$device);
			$r["date"] = $v;
            $r["rate"] = $spend["installs"]>0?round($r["num"]*100/$spend["installs"],2)."%":"0";	
			$revenue_info = $this->getRevenue($app_id,$date,$v,$v,$country,$media_source,$ad_source,$campaign_id,$device);
			$total_revenue+=$revenue_info["total_revenue"];
			$r["ad_source_list"] = $revenue_info["revenueList"];
			$r["revenue"] = $revenue_info["total_revenue"];
			$r["Reward"] = $this->getnewAdtypeImpression($app_id,$date,$v,$v,$country,$media_source,$ad_source,$campaign_id,$device,"Reward");
			$r["avgReward"] = $r["num"]>0?round($r["Reward"]/$r["num"],2):"0";			
			$r["Inter"] = $this->getnewAdtypeImpression($app_id,$date,$v,$v,$country,$media_source,$ad_source,$campaign_id,$device,"Inter");
			$r["avgInter"] = $r["num"]>0?round($r["Inter"]/$r["num"],2):"0";
			$r["avgImpression"] = $r["num"]>0?round($r["impression"]/$r["num"],2):"0";
			$r["total_revenue"] = $total_revenue;
			$r["total_roi"] = $spend["spend"]>0?round($total_revenue*100/$spend["spend"],2)."%":0;
			$r["day_roi"] = $spend["spend"]>0?round($revenue_info["total_revenue"]*100/$spend["spend"],2)."%":0;
			$r["day_avg_revenue"] = $spend["installs"]>0?round($revenue_info["total_revenue"]/$spend["installs"],3):0;
			$r["total_avg_revenue"] = $spend["installs"]>0?round($total_revenue/$spend["installs"],3):0;
            $r["index"] = $key;
            $out_data[] = $r;
		}
		$out =array(
		   ...
		   "cpi"=>$cpi,
		   "promote_info"=>$spend,
		   "user_value"=>$this->get_one_rows($app_id,$date,$start,$end,$country,$media_source,$ad_source,$campaign_id,$device),
		   "roi_info"=>end($out_data),
		);
		$out["rate"] = $out["promote_info"]["installs"]>0?round($out["user_value"]["num"]*100/$out["promote_info"]["installs"],2)."%":"0";
		$out["avgImpression"] = $out["user_value"]["num"]>0?round($out["user_value"]["impression"]/$out["user_value"]["num"],2):"0";
		$out["avg_revenue"] = $spend["installs"]>0?round($out["roi_info"]["total_revenue"]/$spend["installs"],3):"0";
		
		$Reward= $this->getnewAdtypeImpression($app_id,$date,$start,$end,$country,$media_source,$ad_source,$campaign_id,$device,"Reward");
		$out["avgReward"] = $spend["installs"]>0?round($Reward/$spend["installs"],2):"0";
		$Inter = $this->getnewAdtypeImpression($app_id,$date,$start,$end,$country,$media_source,$ad_source,$campaign_id,$device,"Inter");
		$out["avgInter"] = $spend["installs"]>0?round($Inter/$spend["installs"],2):"0";
		echo json_encode($out);exit;
	}
	...
	private function get_one_rows($app_id,$date,$start,$end,$country,$media_source,$ad_source,$campaign_id,$device){
		...
		$row= Db::query("SELECT count(*) as num,ifnull(SUM(num),0) as impression from (
SELECT af_id,ad_source,adtype,count(*) as num from hellowd_appsflyer where {$where} GROUP BY af_id ) c");
		if( !empty($row) && isset($row[0]) )
		{
			return $row[0];
		}
		return ["num"=>0,"impression"=>0];
	}
	...
	private function byChannel($app_id,$date,$start,$end,$country,$media_source,$ad_source,$campaign_id,$device,$table)
	{
		...
		$row=Db::query("SELECT ad_source,adtype,sum(num) as num from hellowd_{$table}_impression where {$where} GROUP BY ad_source,adtype");
		return $row;
	}
	
	private function getnewAdtypeImpression($app_id,$date,$start,$end,$country,$media_source,$ad_source,$campaign_id,$device,$adtype=""){
		
		$dates = getDateFromRange($start, $end);
		$num=0;
		foreach( $dates as $key=>$v )
		{
			$num+=$this->getAdtypeImpression($app_id,$date,$v,$v,$country,$media_source,$ad_source,$campaign_id,$device,$adtype);
		}
		return $num;
	}
	
	private function getAdtypeImpression($app_id,$date,$start,$end,$country,$media_source,$ad_source,$campaign_id,$device,$adtype="")
	{
		$where="gb_id={$app_id} and install_date='{$date}' and event_date>='{$start}' and event_date<='{$end}'";
		if( $country!="all" )
		{
		...
		$num = Db::name("appsflyer_impression")->where($where)->sum('num');
		return $num;
	}
