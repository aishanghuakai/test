...
// 应用公共文件
 ...
 function getmyallowapps()
 {
	 $ids =getmylikedata();		
	 $where=" id in(".$ids.")";
	 $apps= Db::name("app")->field("id,app_name,platform,unique_hash")->where($where)->order("FIELD(id,{$ids})")->select();
	 return $apps;
 }
 ...
 function permission($rule_id="")
 {
	...
	if( in_array($admin_userinfo["ad_role"],["super","publisher"] ) )
	{
		return true;
	}
 ...
 function isshowtips($appid,$date,$tag)
 {
	...
	if(!empty($r) )
	{
		...
		return "<p>{$r['title']}<p><p>".$r["content"]."</p>";
	}
	return 0;
 }
 ...
 function getapp_name($appid="")
 {
	...
	if( !$appid )exit("不合法操作");
	$r = Db::name("app")->field("app_name")->find($appid);
	return $r["app_name"];
 }
